...
async def run_agent_analysis(db: AsyncSession, triage_summary: dict) -> str | None:
    ...
    """
    if not triage_summary.get("suppliers_at_risk") and not triage_summary.get(
        "regional_escalations"
    ):
        return None
    ...
    parts = ["AUTOMATED TRIAGE REPORT — analyze these findings and provide recommendations:\n"]
